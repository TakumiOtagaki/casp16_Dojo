...
def pdb_formater(input_file, output_file, remove_first_residue=True, verbose=False):
    ...
    for model in structure:
        ...
        for chain in model:
            ...
            new_chain_id = chain_id_map.get(chain.id)
            if new_chain_id is None:
                if chain.id.isalpha():
                    new_chain_id = chr(ord('A') + chain_counter)
                else:
                    new_chain_id = str(chain_counter)
                chain_id_map[chain.id] = new_chain_id
                chain_counter += 1
            ...
            for residue in chain.get_residues():
                ...
                for atom in residue.get_atoms():
                    if atom.element == 'H':
                        vprint(f"Atom {atom.name.strip()} in residue {residue.resname} is a hydrogen atom: id={atom.id}, serial_number={atom.serial_number}")
